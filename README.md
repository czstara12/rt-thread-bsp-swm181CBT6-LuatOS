...
本 BSP 的 GPIO 与外设引脚配置统一使用 **RT-Thread pin 名称字符串**，应用层和 `menuconfig` 中都推荐使用 `PA0`、`PB8` 这类名称，再由 `rt_pin_get()` 在初始化阶段解析。

| **片上外设**        | **支持情况**  | **备注**                              |
| :----------------- | :----------: | :----------------------------------- |
| GPIO               |     支持     | 支持 `PA0`~`PE15` 命名及中断映射        |
| UART               |     支持     | UART0~UART3 多实例，引脚名可灵活配置    |
...
| RTC                |   不支持     | 软 RTC 已移除，暂未提供硬件 RTC 驱动    |
